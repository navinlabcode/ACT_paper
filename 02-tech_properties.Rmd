...
tn5_cov <- calc_coverage(path = here("extdata/covhist/TN5/")) %>% 
  mutate(sample = "TN5",
         tech = "ACT")

tn6_cov <- calc_coverage(path = here("extdata/covhist/TN6/")) %>% 
  mutate(sample = "TN6",
         tech = "ACT")

tn7_cov <- calc_coverage(path = here("extdata/covhist/TN7/")) %>% 
  mutate(sample = "TN7",
         tech = "ACT")

tn8_cov <- calc_coverage(path = here("extdata/covhist/TN8/")) %>% 
  mutate(sample = "TN8",
         tech = "ACT")
...
TN5_bincounts <- readRDS(here("extdata/bincounts/TN5_bincounts.rds"))

TN6_bincounts <- readRDS(here("extdata/bincounts/TN6_bincounts.rds"))

TN7_bincounts <- readRDS(here("extdata/bincounts/TN7_bincounts.rds"))

TN8_bincounts <- readRDS(here("extdata/bincounts/TN8_bincounts.rds"))
...
TN1_overdispersion <-  map_dfr(TN1_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN1",
         tech = "ACT",
         cells = names(TN1_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")

TN2_overdispersion <-  map_dfr(TN2_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN2",
         tech = "ACT",
         cells = names(TN2_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")

TN3_overdispersion <-  map_dfr(TN3_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN3",
         tech = "ACT",
         cells = names(TN3_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")

TN4_overdispersion <-  map_dfr(TN4_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN4",
         tech = "ACT",
         cells = names(TN4_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")

TN5_overdispersion <-  map_dfr(TN5_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN5",
         tech = "ACT",
         cells = names(TN5_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")

TN6_overdispersion <-  map_dfr(TN6_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN6",
         tech = "ACT",
         cells = names(TN6_bincounts)[-c(1:3)]) %>% 
  mutate(cells = str_replace_all(cells, "\\.", "-")) %>% 
  dplyr::rename(iod = "V1")

TN7_overdispersion <-  map_dfr(TN7_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN7",
         tech = "ACT",
         cells = names(TN7_bincounts)[-c(1:3)]) %>% 
  mutate(cells = str_replace_all(cells, "\\.", "-")) %>% 
  dplyr::rename(iod = "V1")

TN8_overdispersion <-  map_dfr(TN8_bincounts[,-c(1:3)], 
                    overdispersion) %>%
  t() %>%
  as.data.frame() %>% 
  mutate(sample = "TN8",
         tech = "ACT",
         cells = names(TN8_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")

#10X CNA
names(TN1_10XCNA_bincounts) <- str_replace(names(TN1_10XCNA_bincounts), "\\.", "-")

TN1_10XCNA_overdispersion <- map_dfr(TN1_10XCNA_bincounts[,-c(1:3)], 
                          overdispersion) %>% t() %>% as.data.frame() %>% 
  mutate(sample = "TN1",
         tech = "10X CNA",
         cells = names(TN1_10XCNA_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")

names(TN3_10XCNA_bincounts) <- str_replace(names(TN3_10XCNA_bincounts), "\\.", "-")

TN3_10XCNA_overdispersion <- map_dfr(TN3_10XCNA_bincounts[,-c(1:3)], 
                          overdispersion) %>% t() %>% as.data.frame() %>% 
  mutate(sample = "TN3",
         tech = "10X CNA",
         cells = names(TN3_10XCNA_bincounts)[-c(1:3)]) %>% 
  dplyr::rename(iod = "V1")
