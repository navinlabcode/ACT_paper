...
popseg_bed <- bins_in_cna_pipeline %>%
  makeGRangesFromDataFrame(keep.extra.columns = T, ignore.strand = T)

olaps <- findOverlaps(popseg_bed, cytoband)

olaps_df <- as.data.frame(olaps) %>%
  distinct(queryHits, .keep_all = T)

mk_df <- tibble(pos = olaps_df$queryHits,
                chrom = str_extract(seqnames(cytoband), "([0-9]+|[XY])")[olaps_df$subjectHits],
                                arm = str_extract(cytoband$name[olaps_df$subjectHits], "[pq]")) %>%
  mutate(chrom_and_arm = paste0(chrom, arm))

bins_in_cna_pipeline <- bins_in_cna_pipeline %>%
  mutate(chrom_and_arm = mk_df$chrom_and_arm)
